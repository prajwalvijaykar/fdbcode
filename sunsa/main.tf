...
resource "azurerm_key_vault_secret" "openai_subscription_key_new" {
   name         = "OPENAI-API-KEY"
   value        = azurerm_cognitive_account.openai.primary_access_key
   key_vault_id = azurerm_key_vault.key-vault.id
   depends_on = [azurerm_cognitive_account.openai]
...
resource "azurerm_key_vault_secret" "openai_endpoint_new" {
...
resource "azurerm_key_vault_secret" "vision_endpoint" {
   name         = "VISIONAI-ENDPOINT"
   value        = azurerm_cognitive_account.computer_vision.primary_access_key
   key_vault_id = azurerm_key_vault.key-vault.id
   depends_on = [azurerm_cognitive_account.computer_vision]
}
resource "azurerm_key_vault_secret" "windows_secret" {
   name         = "windows-authentication-token"
   value        = "LydSjXfl2mhHDkr62OmEDMpylFOzoc7GIThDazIgr029rxscePIjxhwzXdAqH0gtWidL5kmCvjRJyEbGhbQ13iJEqzH8c80A24qs5S7V2"
   key_vault_id = azurerm_key_vault.key-vault.id
}
...
resource "azurerm_key_vault_secret" "openai_type" {
...
resource "azurerm_key_vault_secret" "deployment_name" {
   name         = "DEPLOYMENT-NAME"
   value        = azurerm_cognitive_deployment.openai_deployment.name
   key_vault_id = azurerm_key_vault.key-vault.id
   depends_on = [azurerm_cognitive_deployment.openai_deployment]
}
